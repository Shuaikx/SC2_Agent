...
sys.path.append(r"/skill_library")
sys.path.append(r"/")
from skill_library.base_skill import build_worker
import random
import cv2
import math
import numpy as np
...
class IncrediBot(BotAI):  # inhereits from BotAI (part of BurnySC2)
    async def on_step(self, iteration: int):  # on_step is a method that is called every step of the game.
        ...
        # if game_time_seconds - times * 60 > 10:
        action = state_rwd_action['action']
        #
        # if int(game_time_seconds) == 2:
        #     fullscreen()
        #     print("full screen")
        #     time.sleep(1)
        #
        # if int(game_time_seconds) % 10 == 0 and game_time_seconds > 5:
        #     filename = screenshot()
        #     moveto_menu()
        #     pause_game()
        #     action = replace(prompt, filename)
        #     print(f"重要的action：{action}")
        #     action =1
